...
def worker(remote, parent_remote, worker_id):
    ...
    while True:
        ...
        if cmd == 'step':
            data = data.item()
            assert data in np.arange(49)

            action_tuple0 = ActionTuple(np.array([[]], dtype=np.float32), np.array([[np.random.randint(0, 7)]], dtype=np.int32))
            action_tuple1 = ActionTuple(np.array([[]], dtype=np.float32), np.array([[np.random.randint(0, 7)]], dtype=np.int32))
